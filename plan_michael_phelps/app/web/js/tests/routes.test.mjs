...
test("routes resolves canonical hash", () => {
  const resolved = resolveRoute("#/modulo/sesion");

  assert.equal(resolved.routeId, "sesion");
  assert.equal(resolved.canonicalHash, "#/modulo/sesion");
...
test("routes resolves aliases and legacy hashes", () => {
  ...
  assert.equal(alias.routeId, "hoy");
  assert.equal(alias.canonicalHash, "#/modulo/hoy");
  assert.equal(alias.redirectedFromLegacy, false);

  const legacy = resolveRoute("#step-timer");
  assert.equal(legacy.routeId, "sesion");
  assert.equal(legacy.canonicalHash, "#/modulo/sesion");
...
test("routes uses default route for empty hash without invalid flag", () => {
  ...
  assert.equal(empty.canonicalHash, "#/modulo/hoy");
